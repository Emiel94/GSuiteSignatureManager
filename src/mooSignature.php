...
//Manipulate data functions
    ...
    public function functionStripUserAttributes($user_array) {
        ...
        foreach ($this->alias_array as $key => $value) {
            $count;
            $users[] = ["primaryEmail" => $user_array['primaryEmail'], "alias" => $value, "fullName" => $user_array['name']['fullName'], "givenName" => $user_array['name']['givenName'], "familyName" => $user_array['name']['familyName'], "websites" => $user_array['websites'], "thumbnailPhotoUrl" => $user_array["thumbnailPhotoUrl"]];
    ...
    public function generateTemplate($user_info) {
        ...
        foreach ($user_info as $key => $value) {
            if(strcmp($key, "phone0") === 0 && strcmp(substr($value, 0, 3), "tel") !== 0) {
                $tel = "tel:";
...
// Settings functions
    ...
    public function addSettingMOTD($value) {
        // Set MOTD and signature at the same time - Default True (If MOTD HTML isn't set, nothing will display for the MOTD anyway) $this->settingMOTD = $value;
